...
pub struct ModbusRspPacket<'a> {
    pub modbus_rsp_header: ModbusRspHeader<'a>,
    pub modbus_rsp_payload: ModbusRspPayload<'a>,
}
...
impl<'a> PacketTrait<'a> for ModbusRspPacket<'a> {
    ...
    fn parse(input: &'a [u8]) -> nom::IResult<&'a [u8], Self> {
        let (input, modbus_rsp_header) = Self::parse_header(input)?;
        let (input, modbus_rsp_payload) = Self::parse_payload(input, &modbus_rsp_header)?;
        Ok((input, Self { modbus_rsp_header, modbus_rsp_payload }))
    }            
}
...
pub fn parse_read_file_record_rsp_sub_request(input: &[u8]) -> IResult<&[u8], ReadFileRecordRspSubRequest> {
