...
pub struct TcpPacket<'a> {
    pub tcp_header: TcpHeader<'a>,
    pub tcp_payload: TcpPayload<'a>,
}
...
impl<'a> PacketTrait<'a> for TcpPacket<'a> {
    ...
    fn parse_payload(
        ...
        match input.len() {
            ...
            _ => match _header.src_port {
                0x01f6 => match ModbusRspPacket::parse(input) {
    ...
    fn parse(input: &'a [u8]) -> nom::IResult<&'a [u8], Self> {
        let (input, tcp_header) = Self::parse_header(input)?;
        let (input, tcp_payload) = Self::parse_payload(input, &tcp_header)?;
        Ok((input, Self { tcp_header, tcp_payload }))
    }            
}