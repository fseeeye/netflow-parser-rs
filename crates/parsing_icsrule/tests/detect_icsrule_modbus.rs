...
use parsing_parser::{
    parsers::{EthernetHeader, Ipv4Header, ModbusReqHeader, ModbusRspHeader, TcpHeader},
...
fn detect_modbus_read_discrete_inputs() {
    ...
    assert_eq!(
        modbus_rule.detect(&packet_rsp),
        DetectResult::Hit(RuleAction::Drop)
    );
    assert_eq!(
...
fn detect_modbus_read_exception_status() {
    ...
    let rsp_pdu = parsing_parser::parsers::modbus_rsp::PDU {
        function_code: 7,
        data: parsing_parser::parsers::modbus_rsp::Data::ReadExceptionStatus { output_data: 1 },
    };
    let packet_rsp = make_modbus_rsp_packet(rsp_pdu);
...
fn detect_modbus_read_coil_exc() {
    let rsp_pdu = parsing_parser::parsers::modbus_rsp::PDU {
        function_code: 24,
        data: parsing_parser::parsers::modbus_rsp::Data::ReadCoilsExc { exception_code: 1 },
    };
    let packet_rsp = make_modbus_rsp_packet(rsp_pdu);
