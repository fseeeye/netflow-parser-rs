use parsing_parser::{L5Packet, ApplicationLayer, parsers::opcua};
    ...
    fn detect(&self, l5: &L5Packet) -> bool {
        if let ApplicationLayer::Opcua(opcua) = &l5.application_layer {
            ...
            if let Some(target_service_nodeid_numeric) = self.service_nodeid_numeric {
                if let opcua::MessageTypeEnum::Message { msg_variant_info: opcua::MsgVariantInfo::Service { service_nodeid_info, ..}, ..} = &opcua.message_type_enum {
