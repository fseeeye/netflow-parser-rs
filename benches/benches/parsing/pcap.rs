...
use parsing_rs::{Layer, LayerType, QuinPacket, QuinPacketOptions, VecPacket, VecPacketOptions, parse_quin_packet, parsers_map_init};
...
fn vec_parsing_benchmark_pcap(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("vec_parsing_benchmark");
    group.warm_up_time(std::time::Duration::from_millis(500));
    group.measurement_time(std::time::Duration::from_secs(10));
            ...
            match reader.next() {
                Ok((offset, block)) => {
                    match block {
                        PcapBlockOwned::LegacyHeader(_hdr) => {}
                        PcapBlockOwned::Legacy(_b) => {
                            let mut packet = VecPacket::new(black_box(&_b.data), black_box(VecPacketOptions::new()));
                            packet.parse(black_box(&parsers_map));
...
fn vec_parsing_benchmark_modbus(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("vec_parsing_benchmark");
    group.warm_up_time(std::time::Duration::from_millis(500));
    group.measurement_time(std::time::Duration::from_secs(10));
    group.sample_size(1000);
    ...
    let input = [0, 14, 198, 208, 125, 185, 76, 237, 251, 115, 84, 107, 8, 0, 69, 0, 0, 52, 123, 78, 64, 0, 128, 6, 0, 0, 192, 168, 3, 190, 192, 168, 3, 189, 208, 8, 1, 246, 235, 71, 73, 93, 70, 24, 37, 8, 80, 24, 2, 12, 136, 242, 0, 0, 1, 0, 0, 0, 0, 6, 1, 2, 0, 0, 0, 4];
    let parsers_map = parsers_map_init();
    let options = VecPacketOptions::new();

    group.bench_function("parse_modbus 1 (Request)(func_code: 2)", |bencher| bencher.iter(|| {
        let mut packet = VecPacket::new(&input, options);
...
fn vec_parsing_benchmark_tcp(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("vec_parsing_benchmark");
    group.warm_up_time(std::time::Duration::from_millis(500));
    group.measurement_time(std::time::Duration::from_secs(10));
    group.sample_size(1000);
    ...
    group.bench_function("parse_tcp 1", |bencher| bencher.iter(|| {
        let mut packet = VecPacket::new(&input, options);
        packet.parse(&parsers_map);
    }));
...
fn vec_parsing_benchmark_udp(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("vec_parsing_benchmark");
    group.warm_up_time(std::time::Duration::from_millis(500));
    group.measurement_time(std::time::Duration::from_secs(10));
    group.sample_size(1000);
    ...
    group.bench_function("parse_udp 1", |bencher| bencher.iter(|| {
        let mut packet = VecPacket::new(&input, options);
        packet.parse(&parsers_map);
    }));
...
criterion_group!(benches, quin_parsing_benchmark_pcap, vec_parsing_benchmark_pcap, quin_parsing_benchmark_modbus, vec_parsing_benchmark_modbus, quin_parsing_benchmark_tcp, vec_parsing_benchmark_tcp, quin_parsing_benchmark_udp, vec_parsing_benchmark_udp);
